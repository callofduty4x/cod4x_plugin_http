...
align(1) struct reliableprotocol_t{
    void *netstate;
    msg_t recvbuffer;
}
...
align(1)://90b4f8c
	...
	byte[MAX_GENTITIES / 8]			entityNotSolid;//One bit for entity  - No use
	byte[MAX_GENTITIES / 8]			entityInvisible;//One bit for entity  - No use
	uint		clFrames;
	...
	uint	updateBeginTime;
	byte[40 + 44]	reliablemsg;
	ulong		steamid;
	ulong		steamidPending;
	ulong		clanid;
	ulong		clanidPending;
	ulong		playerid;
	int			steamstatus;
...
//Unknown where the offset error is
	...
	/*byte[284]*/int		download;		//(0x212e4) file being downloaded
	...
	ubyte* [MAX_DOWNLOAD_WINDOW] dfree1;	//(0x212fc) the buffers for the download blocks
	int[MAX_DOWNLOAD_WINDOW -3]			dfree2;	//(0x2131c)
